...
class FirstPersonCameraDemo {
  ...
  initializeAudio_() {
    ...
    loader.load('resources/music/Avicii - The Nights (Mp3 Download, Lyrics) » Jesusful.mp3', (buffer) => {
      setTimeout(() => {
        sound1.setBuffer(buffer);
        sound1.setLoop(true);
        sound1.setVolume(8.0);
  ...
  initializeScene_() {
    ...
    light = new THREE.SpotLight(
      0xFFdd, 100.0, distance, angle, penumbra, decay);
    ...
    light = new THREE.SpotLight(
      0xffd, 100.0, distance, angle, penumbra, decay);
    light.castShadow = true;
    light.shadow.bias = -0.00001;
    light.shadow.mapSize.width = 4096;
    light.shadow.mapSize.height = 4096;
...
light.shadow.camera.near = 1;
light.shadow.camera.far = 100;
light.position.set(10, 4, -45);
light.target.position.set(5, 2, -30);
this.scene_.add(light);
this.scene_.add(light.target);
    ...
    const texture = loader.load([
        './resources/skybox/Image/rsz_1posz.jpg',
        './resources/skybox/Image/rsz_1posz.jpg',
        './resources/skybox/Image/rsz_2negx.jpg',
        './resources/skybox/Image/rsz_negy.jpg',
        './resources/skybox/Image/rsz_1posz.jpg',
        './resources/skybox/Image/rsz_1posz.jpg',
    ]);
    ...
    const speaker3 = new THREE.Mesh(
      new THREE.BoxGeometry(20, 20, -2.5),
      new THREE.MeshStandardMaterial({color: 0x404040, roughness: 0.1, metalness: 0 }));
      speaker3.position.set(10, 4, 40);
    speaker3.castShadow = true;
    speaker3.receiveShadow = true;
    this.scene_.add(speaker3);

    this.speakerMesh3_ = speaker3;
    

    const speaker4 = new THREE.Mesh(
      new THREE.BoxGeometry(10, 10, -2.5),
      new THREE.MeshStandardMaterial({color: 0x404040, roughness: 0.1, metalness: 0 }));
      speaker4.position.set(10, 4, -45);
    speaker4.castShadow = true;
    speaker4.receiveShadow = true;
    this.scene_.add(speaker4);

    this.speakerMesh4_ = speaker4;
...
//speaker Animation
    ...
    this.speaker3Material_ = visualizerMaterial;

    const speaker3Screen = new THREE.Mesh(
      new THREE.PlaneGeometry(50, 20),
      this.speaker3Material_);
    speaker3Screen.castShadow = false;
    speaker3Screen.receiveShadow = true;
    speaker3Screen.rotation.y = -Math.PI;
    speaker3Screen.position.x -= 5;
    speaker3Screen.position.z -= 2;
    this.speakerMesh3_.add(speaker3Screen);



    this.speaker4Material_ = visualizerMaterial;

    const speaker4Screen = new THREE.Mesh(
      new THREE.PlaneGeometry(50, 20),
      this.speaker4Material_);
    speaker4Screen.castShadow = false;
    speaker4Screen.receiveShadow = true;
    speaker4Screen.rotation.y -= 0.08;
    speaker4Screen.position.x -= 4;
    speaker4Screen.position.z = 5;
    this.speakerMesh4_.add(speaker4Screen);
